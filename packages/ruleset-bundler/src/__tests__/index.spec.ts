...
import { migrateRuleset } from '@stoplight/spectral-ruleset-migrator/src';
...
describe('Ruleset Bundler', () => {
  ...
  //     });
  //
  //     it('and a URL as the input source', async () => {
  //       let input = 'https://raw.githubusercontent.com/stoplightio/spectral-ruleset/develop/spectral.yaml';
  //       serveAssets({
  //         [input]: `extends: spectral:oas
  // rules:
  //   api-home:
  //     description:
  ...
  //       });
  //
  //       const migratedRuleset = await migrateRuleset(input, {
  //         format: 'esm',
  //         ...io,
  //       });
  //
  //       input = input.replace(/\.yaml$/, '.js');
  //       const code = await bundleRuleset(input, {
  //         target: 'runtime',
  //         plugins: [stdin(migratedRuleset, input), ...runtime(io)],
  //       });
  //
  //       expect(code).toEqual('d');
  //     });
  //   });
  ...
  describe('given node target', () => {
    ...
    it('and a URL as the input source', async () => {
      const input = 'https://raw.githubusercontent.com/mkistler/azure-spectral-ruleset/main/spectral.yaml';
      ...
      const code = await bundleRuleset('/.spectral.js', {
        target: 'node',
        format: 'commonjs',
        plugins: [stdin(migratedRuleset, '/.spectral.js'), ...node(io)],
      });
